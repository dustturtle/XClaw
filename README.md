...
4. [配置详解](#配置详解)
5. [对话渠道](#对话渠道)
6. [Web API 参考](#web-api-参考)
7. [工具参考](#工具参考)
8. [Skills 系统](#skills-系统)
9. [MCP 工具联邦](#mcp-工具联邦)
10. [投资回测](#投资回测)
11. [智能记忆](#智能记忆)
12. [多用户隔离](#多用户隔离)
13. [定时任务](#定时任务)
14. [Docker 部署](#docker-部署)
15. [安全设计](#安全设计)
16. [开发与测试](#开发与测试)
17. [技术栈](#技术栈)
...
### 2026-03-27：A 股数据源重构（方案与验证基线完成）

最近一次提交 `0cb4cd2` 新增了 [`datasource.md`](datasource.md)，把 XClaw 的 A 股数据源从“单一源思路”正式收敛为“按能力拆层 + 主备切换”的方案。这次变更的重点不是再往 README 里堆一个数据源名字，而是明确第一版在当前网络环境下应该怎么稳定落地。

这轮重构的核心结论：

- 不再尝试用单一免费源覆盖历史、实时、标的信息三类能力，而是拆成三个 provider 层分别治理
- 历史行情建议走 `BaoStock -> pytdx -> TickFlow -> yfinance`
- 实时行情建议走 `腾讯直连 HTTP -> 新浪直连 HTTP`
- 标的信息建议走 `TickFlow -> BaoStock -> pytdx`
- 内部统一股票代码格式建议使用 `600519.SH` / `000001.SZ` / `430001.BJ`
- 统一标准输出字段，避免把底层数据源原始字段直接暴露给业务层

这次调研同时明确了第一版不应作为核心依赖的链路：

- `AKShare`
- `efinance`
- `Tushare Pro`
- 强依赖 Eastmoney API 的接口族

原因不是这些项目“完全不能用”，而是 `2026-03-26` 这次实际验证表明：它们在当前机器和网络环境下无法稳定承担 A 股核心主链路，尤其是 Eastmoney 相关接口和依赖新浪 HTTPS 证书链的子路径。

推荐的架构边界如下：

- `HistoricalDataProvider`：负责日线 / 周线 / 月线、回测输入、历史批量拉取
- `RealtimeQuoteProvider`：负责单票实时、小批量快照和盘中行情
- `InstrumentProvider`：负责股票代码、名称、交易所、上市日期等静态元数据

当前仓库状态也需要说明白：

- 本轮已经完成 A 股数据源的选型、验证报告、failover 策略和推荐模块结构，详见 [`datasource.md`](datasource.md)
- 现有投资工具仍有一部分是直接调用 `akshare` / `yfinance` 的实现
- 下一步的代码演进方向，是把这些工具逐步收敛到统一的 datasource adapters / managers，而不是继续在业务工具里散落源站细节

如果你现在是以“先稳定跑起来”为目标，README 推荐直接按这条基线理解和推进：

- 历史主链路：`BaoStock + pytdx`
- 实时主链路：`腾讯直连 HTTP + 新浪直连 HTTP`
- 标的信息主链路：`TickFlow`
- 海外兜底：`yfinance`
